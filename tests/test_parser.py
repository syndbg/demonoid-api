...
class OfflineParserTests(TestCase):
    """
        ...
        Offline page created in 2015-03-12.
    """

    @classmethod
    def setUpClass(cls):
        with open(HERE + '/data/files.html') as f:
            files_html = f.read()
        httpretty.register_uri(httpretty.GET, Url.DEFAULT_BASE_URL + 'files', body=files_html)
    ...
    def test_parse_first_row_without_external_torrent_property(self):
        first_torrent_row = self.rows[1]

        result = Parser.parse_first_row(first_torrent_row, self.url)
        self.assertEqual(5, len(result))
        self.assertEqual('012345/012345678901/', result[0])
        self.assertEqual('', result[1])
        self.assertEqual('Demonoid', result[2])
        self.assertEqual('', result[3])
        self.assertEqual(self.url.combine(''), result[4])
...
class OnlineParserTests(TestCase):
    """
       ...
       However not every bit of code can be tested against the LIVE page.
       That's why there's a lot of mocked behavior in OnlineParserTests.
    """
